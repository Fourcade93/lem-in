...
/*   Updated: 2018/01/30 16:43:55 by fmallaba         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_list	*search_ways(t_list *connect, t_list *tmp, t_list *buf)
{
	while (connect)
	{
		if (ft_strequ(((t_room *)(connect->content))->tag, END))
			put_weight_to_end(((t_room *)
			(connect->content))->connect, tmp);
		else if ((((t_room *)(connect->content))->weight >
				((t_room *)(tmp->content))->weight + 1 ||
				((t_room *)(connect->content))->weight == 0) &&
				!ft_strequ(((t_room *)(connect->content))->tag, START))
		{
			((t_room *)(connect->content))->weight =
				((t_room *)(tmp->content))->weight + 1;
